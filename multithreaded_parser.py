...
def get_online_sellers(root):
    for s in root.find_all(class_="tc-item", attrs={"data-server": server_id}):
        seller = Seller(s.find("div", class_="tc-user").find(class_="media-user-name").text.strip(), s['href'], int(s.find(class_="tc-amount").text.replace(" ", "")),
...
def get_best_sellers(root):
    ...
    if not input_price.get().isnumeric():
        input_price.set(0)

    if not input_in_stock.get().isnumeric():
